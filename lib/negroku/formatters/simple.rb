  ...
  module Formatter
    ...
    class Simple < Abstract
      ...
      def write_command(command)
        ...
        if SSHKit.config.output_verbosity == Logger::INFO
          unless command.stdout.empty?
            command.stdout.lines.each do |line|
              original_output << "%6s %s" % [level(Logger::INFO),
                                             c.red("\t" + line)]
              original_output << "\n" unless line[-1] == "\n"
            end
          end
        ...
        if command.finished?
          original_output << "%6s %s\n" % [level(command.verbosity),
                                           uuid(command) + "Finished in #{sprintf('%5.3f seconds', command.runtime)} with exit status #{command.exit_status} (#{c.bold { command.failure? ? c.red('failed') : c.red('successful') }})."]
        end
      end
